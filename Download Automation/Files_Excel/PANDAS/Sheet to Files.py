...
if choice == 1:
    ...
    for sht in excel_file.sheet_names:
        ...
        for category in df[col_name].unique():
            ...
            category_file = os.path.join(ofolder_path, f"{category}.xlsx")
            newDf.to_excel(category_file, index=False)
...
##########################################################################
    ...
    for category, data in combined_data.items():
        category_file = os.path.join(output_folder, f"{category}.xlsx")
        combined_category = pd.concat(data, ignore_index=True)
        combined_category.to_excel(category_file, index=False)

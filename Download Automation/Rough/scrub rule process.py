...
def fx_ncpt_rptvst(row):
    if 1 <= row['Days'] <= 7:
        newcpt =  None
        rptvst =  f"Repeat Visit [Before 7 Days] : {row['PATCODE']} - First Visit on {row['BILLDATE_Min'].strftime('%d/%m/%Y')}"
    elif 8 <= row['Days'] <= 31:
        newcpt = str(int(row['CPTCODE'])+ .02)
        rptvst = f"Repeat Visit [After 7 Days] : {row['PATCODE']} - First Visit on {row['BILLDATE_Min'].strftime('%d/%m/%Y')}"
    elif row['Days'] == 0:
        newcpt = str(int(row['CPTCODE']))
        rptvst = None
    return pd.Series([newcpt, rptvst])

cptchange_df[['NewCPTCODE1', 'Repeat Visit']] = cptchange_df.apply(fx_ncpt_rptvst, axis=1)
